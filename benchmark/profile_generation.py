...
def profile_throughput(
    ...
    output_seqlen = gen_config.max_new_tokens
    print(
        f"profiling ... concurrency: {concurrency}, "
    ...
    for i in range(concurrency):
        proc = Thread(
            target=infer, args=(tm_model, i + 1, input_ids, gen_config, test_round, que)
        )
        procs.append(proc)
        proc.start()
    ...
    total_token_throughput = np.round(
        concurrency * test_round * (input_seqlen + output_seqlen) / elapsed_time, 2
    )
    print(
    ...
    return (
        model_path,
        [first_token_latency_min, first_token_latency_max, first_token_latency_ave],
        percentiles,
        out_token_throughput,
...
def parse_args():
    ...
    parser.add_argument(
        "-tr", "--test-round", type=int, help="number of test rounds", default=3
    )
    parser.add_argument(
        "-w", "--warmup-round", type=int, help="number of warmup rounds", default=1
    )
...
def _process_map(target, iterable):
    ...
    proc = spawn_context.Process(
        target=__proc_cb, args=iterable, kwargs=dict(ret_pipe=pipe, target=target)
    )
    proc.start()
    proc.join()
            ...
            for re in results:
                writer.writerow(
                    [
                        ...
                        f"{re.output_throughput:.2f}",
                        f"{re.mem_per_gpu:.2f}",
                        re.first_token_latency[2],
                        re.first_token_latency[0],
                        re.first_token_latency[1],
                        re.percentiles[0],
                        re.percentiles[1],
                        re.percentiles[2],
                        re.percentiles[3],
