...
async def benchmark(
    ...
    if args.output_file:
        ...
        if args.dataset_name == "random":
            ...
            output_file_name = (
                f"{args.backend}_{now}_{args.num_prompts}_sharegpt.jsonl"  # noqa
            )
...
def run_benchmark(args_: argparse.Namespace):
    ...
    if args.backend in ["sglang", "sglang-native"]:
        ...
        api_url = (
            ...
            if args.base_url
            else f"http://{args.host}:{args.port}/v2/models/ensemble/generate_stream"  # noqa
        )
    ...
    if args.model is None:
        try:
            ...
            print(
                "Please specify the correct host and port using "
                "`--host` and `--port`."
            )
            sys.exit(1)
