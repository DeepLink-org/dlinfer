...
**注意：** 如果引入新的随 batch size 变化的 tensor，三个方法都需要同步更
新：`make_buffers` 负责分配，`fill_buffers` 负责填充，`update_context`
负责让 `StepContext` 指向它。
...
GraphRunner.__call__
  └─ compatible_size = get_compatible_size(batch_size)
       └─ _runner_map[compatible_size] 不存在 → 创建 CUDASingleGraphRunner
            （Ascend 使用 AscendSingleGraphRunner）
            │
...
> vendor，与 lmdeploy 默认实现相同，只需作为 note 了解即可。

---

## Ascend —— Replay 期间的 kv_seqlens 更新

Ascend NPU graph replay 要求将每个 decode 步骤都会变化的动态值（如
`kv_seqlens`）原地更新到已捕获的图中，而不是写入 input buffer 后重新捕获。
...
| `fill_buffers` 中 `max_kv_seq_len` | 保持原值 | 设为 -1 | 设为 -1 |
...
2. **Camb 和 MACA 的 `max_kv_seq_len` 必须设为 -1。** 此整数在捕获时被
   烘焙为常量。若 capture 时填入真实的最大值，replay 时每一步都会出错；
   设为 -1 则告知 attention kernel 动态计算最大值。
