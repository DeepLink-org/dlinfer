...
class AtenToAtbTransformer(SingleOpTransformer):
    ...
    def dlinfer_fused_moe(
        ...
        moe_init = self.get_proxy(
            atb_op.MoeInitRouting, (hidden_states, row_ids, topk_ids, active_num)
        )
        ...
        up_proj = self.get_proxy(atb_op.GetItem, (up_sample, 1))
        ...
        down_proj = self.get_proxy(atb_op.GetItem, (down_sample, 1))

        # finalize routing
        topk_weights = self.get_proxy(atb_op.Cast, (topk_weights, hidden_states_dtype))
        skip = self.get_proxy(atb_op.ZerosLike, (hidden_states,))
        bias = self.get_proxy(atb_op.ZerosLike, (down_proj,))
        export_for_source_row = self.get_proxy(atb_op.ZerosLike, (topk_ids,))
        moe_out = self.get_proxy(
            atb_op.MoeFinalizeRouting,
            (
                down_proj,
                skip,
                skip,
