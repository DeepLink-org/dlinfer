...
atb::Status PrepareMoeOperation::InferShape(const atb::SVector<atb::TensorDesc>& inTensorDescs, atb::SVector<atb::TensorDesc>& outTensorDescs) const {
    ...
    outTensorDescs.at(2).shape.dims[0] = inTensorDescs.at(0).shape.dims[0] *  inTensorDescs.at(0).shape.dims[1];
    ...
    outTensorDescs.at(3).shape.dims[0] = inTensorDescs.at(0).shape.dims[0] *  inTensorDescs.at(0).shape.dims[1];
...
int PrepareMoeOperation::Setup(const atb::VariantPack& variantPack, uint64_t& workspaceSize, atb::Context* context) {
    ...
    DICP_LOG(INFO) << opName_ << " aclnnArangeGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << arangeWorkspaceSize_ << ", aclExecutor:" << aclArangeExecutor_;
    ...
    DICP_LOG(INFO) << opName_ << " aclnnPermuteGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << permuteWorkspaceSize_ << ", aclExecutor:" << aclPermuteExecutor_;
    ...
    ret = aclnnBincountGetWorkspaceSize(aclInTensors_.at(0).tensor, nullptr, numExperts_, aclOutTensors_.at(2).tensor, &bincountWorkspaceSize_, &aclBincountExecutor_);
    DICP_LOG(INFO) << opName_ << " aclnnBincountGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << bincountWorkspaceSize_ << ", aclExecutor:" << aclBincountExecutor_;
    ...
    ret = aclnnCumsumGetWorkspaceSize(aclOutTensors_.at(2).tensor, 0, aclDataType::ACL_INT64, aclOutTensors_.at(3).tensor, &cumsumWorkspaceSize_, &aclCumsumExecutor_);
    DICP_LOG(INFO) << opName_ << " aclnnCumsumGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << cumsumWorkspaceSize_ << ", aclExecutor:" << aclCumsumExecutor_;
