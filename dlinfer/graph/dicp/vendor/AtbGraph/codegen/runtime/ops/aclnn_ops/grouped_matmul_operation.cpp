...
AclNnGroupedMatmulOperation::AclNnGroupedMatmulOperation(const std::string& name, int64_t splitItem)
    : opName_(name), splitItem_(splitItem) {}
...
int AclNnGroupedMatmulOperation::Setup(const atb::VariantPack& variantPack, uint64_t& workspaceSize, atb::Context* context) {
    ...
    int ret = aclnnPermuteGetWorkspaceSize(aclInTensors_.at(1).tensor, permuteDims, aclOutTensors_.at(0).tensor, &permuteWorkspaceSize_, &aclPermuteExecutor_);
    ...
    ret = aclnnGroupedMatmulV3GetWorkspaceSize(
        aclInTensorList_.at(0), aclOutTensorList_.at(0), nullptr, nullptr, nullptr, nullptr, nullptr, aclInTensors_.at(2).tensor, splitItem_, 0, aclOutTensorList_.at(1), &groupedMatmulWorkspaceSize_, &aclGroupedMatmulExecutor_);
    workspaceSize = groupedMatmulWorkspaceSize_ > workspaceSize ? groupedMatmulWorkspaceSize_ : workspaceSize;
    DICP_LOG(INFO) << opName_ << " aclnnGroupedMatmulGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << groupedMatmulWorkspaceSize_ << ", aclExecutor:" << aclGroupedMatmulExecutor_;
