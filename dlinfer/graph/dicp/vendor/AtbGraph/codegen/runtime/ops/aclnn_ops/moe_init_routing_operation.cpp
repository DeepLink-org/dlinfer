...
AclNnMoeInitRoutingOperation::AclNnMoeInitRoutingOperation(const std::string& name, int64_t activeNum) : AclNnOperation(name), activeNum_(activeNum) {}
...
atb::Operation* AclNnMoeInitRoutingOperationCreate(const nlohmann::json& paramJson) {
    ...
    DICP_LOG(INFO) << "AclNnMoeInitRoutingOperation: name: " << opName << " activeNum:" << activeNum;
    atb::Operation* op = new AclNnMoeInitRoutingOperation(opName, activeNum);
    return op;
}
