...
int RenormalizeOperation::Setup(const atb::VariantPack& variantPack, uint64_t& workspaceSize, atb::Context* context) {
    ...
    int ret = aclnnReduceSumGetWorkspaceSize(aclInTensors_.at(0).tensor, reduceDims, true, reduceSumDtype, aclOutTensors_.at(0).tensor, &reduceSumWorkspaceSize_, &aclReduceSumExecutor_);
    DICP_LOG(INFO) << opName_ << " aclnnReduceSumGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << reduceSumWorkspaceSize_ << ", aclExecutor:" << aclReduceSumExecutor_;
