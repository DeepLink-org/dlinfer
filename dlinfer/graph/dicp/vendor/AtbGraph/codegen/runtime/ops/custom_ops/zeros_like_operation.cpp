...
ZerosLikeOperation::ZerosLikeOperation(const std::string& name)
...
int ZerosLikeOperation::SetAclNnWorkspaceExecutor(uint64_t& workspaceSize) {
    ...
    DICP_LOG(INFO) << opName_ << " aclnnInplaceZeroGetWorkspaceSize end, ret:" << ret << ", workspaceSize:" << workspaceSize << ", aclExecutor:" << aclExecutor_;
