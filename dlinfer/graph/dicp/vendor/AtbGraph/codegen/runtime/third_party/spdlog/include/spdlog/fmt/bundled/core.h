...
#if (FMT_HAS_FEATURE(cxx_relaxed_constexpr) || FMT_MSC_VERSION >= 1912 || (FMT_GCC_VERSION >= 600 && FMT_CPLUSPLUS >= 201402L)) && !FMT_ICC_VERSION && \
...
#define FMT_CONSTEXPR
    ...
    ((!defined(_GLIBCXX_RELEASE) || _GLIBCXX_RELEASE >= 10) && (!defined(_LIBCPP_VERSION) || _LIBCPP_VERSION >= 10000) && \
...
#if ((FMT_GCC_VERSION >= 1000 || FMT_CLANG_VERSION >= 1101) && (!defined(__apple_build_version__) || __apple_build_version__ >= 14000029L) && \
...
#if defined(__cpp_nontype_template_args) && ((FMT_GCC_VERSION >= 903 && FMT_CPLUSPLUS >= 201709L) || __cpp_nontype_template_args >= 201911L) && \
...
public:
    ...
    basic_string_view(const Char* s)
        : data_(s),
          size_(detail::const_check(std::is_same<Char, char>::value && !detail::is_constant_evaluated(true)) ? std::strlen(reinterpret_cast<const char*>(s))
...
#if !defined(__LCC__)
        if (arg_id < num_args_ && types_ && !is_integral_type(types_[arg_id])) throw_format_error("width/precision is not integer");
#endif
    }
};
...
struct arg_mapper {
    ...
    template <typename T, typename U = remove_const_t<T>,
              FMT_ENABLE_IF((std::is_class<U>::value || std::is_enum<U>::value || std::is_union<U>::value) && !is_string<U>::value && !is_char<U>::value &&
                            !is_named_arg<U>::value && !std::is_arithmetic<format_as_t<U>>::value)>
...
template <typename Char>
FMT_CONSTEXPR auto parse_dynamic_spec(const Char* begin, const Char* end, int& value, arg_ref<Char>& ref, basic_format_parse_context<Char>& ctx)
    -> const Char* {
    FMT_ASSERT(begin != end, "");
    if ('0' <= *begin && *begin <= '9') {
...
template <typename Char>
FMT_CONSTEXPR FMT_INLINE auto parse_format_specs(const Char* begin, const Char* end, dynamic_format_specs<Char>& specs, basic_format_parse_context<Char>& ctx,
...
FMT_CONSTEXPR auto check_char_specs(const format_specs<Char>& specs) -> bool {
    ...
    if (specs.align == align::numeric || specs.sign != sign::none || specs.alt) throw_format_error("invalid format specifier for char");
    return true;
}
