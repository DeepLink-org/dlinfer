...
class AscendPiecewiseGraphWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        self._canonical_signature: Optional[Tuple[Tuple[str, Tuple[int, ...]], ...]] = None

        logger.debug("ACLGraphWrapper created: first=%s, last=%s, is_decoding=%s, auto_detect=%s",
                     is_first_graph, is_last_graph, is_decoding, self.auto_detect_stage)

    def _build_signature(self, args: Tuple[Any, ...], kwargs: Dict[str, Any]) -> Tuple[Tuple[str, Tuple[int, ...]], ...]:
    ...
    def _ensure_signature(self, args: Tuple[Any, ...], kwargs: Dict[str, Any]) -> Tuple[Tuple[str, Tuple[int, ...]], ...]:
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if cache_key not in self.cache:
                if is_debug_enabled():
                    logger.info("Decode stage: capturing ACL Graph (signature=%s)",
                               self._canonical_signature)
                return self._capture(cache_key, args, kwargs)

            self._debug_log("Decode stage: replaying ACL Graph (signature=%s)",
                           self._canonical_signature)
    ...
    def _generate_cache_key(self, args: Tuple[Any, ...], kwargs: Dict[str, Any]) -> Tuple[bool, ...]:
        return (True,)
    
    def _capture(self, cache_key: Tuple[Any, ...], args: Tuple[Any, ...], kwargs: Dict[str, Any]) -> Any:
        ...
        tensor_args = [(idx, arg) for idx, arg in enumerate(args) if isinstance(arg, torch.Tensor)]
        ...
        try:
            with ExitStack() as stack:
                ...
                with torch.npu.graph(acl_graph, pool=self.graph_pool, stream=current_stream, auto_dispatch_capture=True,):
    ...
    def _replay(self, cache_key: Tuple[Any, ...], args: Tuple[Any, ...], kwargs: Dict[str, Any]) -> Any:
    ...
    def _add_to_cache(self, cache_key: Tuple[Any, ...], entry: ACLGraphEntry) -> None:
        ...
        self.cache[cache_key] = entry
        if is_debug_enabled():
            logger.debug("Added cache entry %s (cache size: %d)", cache_key, len(self.cache))

    def _validate_input_buffers(self, entry: ACLGraphEntry, cache_key: Tuple[Any, ...]) -> None:
        ...
        if new_addresses != entry.input_addresses:
            logger.error(
                ...
                cache_key, entry.input_addresses, new_addresses
            )
            raise RuntimeError("Input buffer addresses changed between capture and replay")

    def _copy_input_data(self, entry: ACLGraphEntry, args: Tuple[Any, ...], cache_key: Tuple[Any, ...]) -> None:
        ...
        for arg_idx, expected_shape in zip(entry.arg_indices, entry.arg_shapes):
            ...
            if new_tensor.shape != expected_shape:
                raise RuntimeError(
                    "Shape mismatch for arg%d: expected %s, got %s" % (arg_idx, expected_shape, new_tensor.shape)
                )
        ...
        for arg_idx, target_view in zip(entry.arg_indices, entry.arg_views):
            ...
            if new_tensor.device != target_view.device:
                raise RuntimeError(
                    "Device mismatch for arg%d: expected %s, got %s" % (arg_idx, target_view.device, new_tensor.device)
                )
