...
def debug_fx_graph_nodes(gm: fx.GraphModule, filter_ops: Optional[List[str]] = None, title: str = "FX Graph Nodes") -> None:
    """
...
def debug_graph_splitting(split_items: List[SplitItem], split_gm: fx.GraphModule) -> None:
    """
    ...
    for item in split_items:
        ...
        logger.info(f"  [{item.graph_id}] {item.submod_name} ({module_type}) - {node_count} nodes")

    # Graph topology analysis
    logger.info(f"\nGraph topology: {split_items[0].graph_id} -> {split_items[-1].graph_id}")
    logger.info("=" * 80)

def debug_submodule_wrapping(split_items: List[SplitItem], split_gm: fx.GraphModule) -> None:
    """
    ...
    for item in split_items:
        wrapper_type = "EagerExecutionWrapper" if item.is_splitting_graph else "AscendPiecewiseGraphWrapper"
        module = getattr(split_gm, item.submod_name)

        # Count operations in this submodule
        node_ops = Counter(node.op for node in module.graph.nodes)
        call_functions = [node for node in module.graph.nodes if node.op == 'call_function']
...
def debug_compilation_summary(
    ...
    total_split_nodes = sum(len(list(getattr(split_gm, item.submod_name).graph.nodes)) for item in split_items)
