...
def fused_lora_impl_abstract_func(
    input: Tensor,
    lora_a: Tensor,
    lora_b: Tensor,
    scaling: Tensor,
    ...
    N = lora_b.size(1)
    if output is None:
        output = input.new_empty((M, N), dtype=input.dtype, device=input.device)
    return output


@register_custom_op(
    "dlinfer::fused_lora",
    impl_abstract_func=fused_lora_impl_abstract_func,
    default_value={"output": None},
)
def fused_lora(
    input: Tensor,
    lora_a: Tensor,
    ...
    Fused lora.
    """
    return vendor_ops_registry["fused_lora"](
        input,
        lora_a,
        lora_b,
        scaling,
