...
def _causal_conv1d_update_kernel_npu_tiled(
    ...
    batch: tl.int32,
    dim: tl.constexpr, seqlen: tl.constexpr, state_len: tl.constexpr,
    ...
    stride_conv_state_seq: tl.constexpr, stride_conv_state_dim: tl.constexpr,
    stride_conv_state_tok: tl.constexpr, stride_state_indices: tl.constexpr,
    stride_o_seq: tl.constexpr, stride_o_dim: tl.constexpr, stride_o_token: tl.constexpr,
    pad_slot_id: tl.constexpr,
    HAS_BIAS: tl.constexpr, KERNEL_WIDTH: tl.constexpr,
    SILU_ACTIVATION: tl.constexpr, IS_VARLEN: tl.constexpr,
    IS_APC_ENABLED: tl.constexpr, IS_SPEC_DECODING: tl.constexpr,
    NP2_STATELEN: tl.constexpr, USE_PAD_SLOT: tl.constexpr,
    BLOCK_N: tl.constexpr, B_TILE: tl.constexpr, T_CHUNK: tl.constexpr,
):
    ...
    for bi in tl.static_range(0, B_TILE):
        ...
        if KERNEL_WIDTH >= 2:
            col0 = tl.load(prior_tokens + 0 * stride_conv_state_tok, mask=lane_active & mask_w, other=0.0).to(tl.float16)
        if KERNEL_WIDTH >= 3:
            col1 = tl.load(prior_tokens + 1 * stride_conv_state_tok, mask=lane_active & mask_w, other=0.0).to(tl.float16)
        if KERNEL_WIDTH >= 4:
            col2 = tl.load(prior_tokens + 2 * stride_conv_state_tok, mask=lane_active & mask_w, other=0.0).to(tl.float16)
        if KERNEL_WIDTH >= 5:
            col3 = tl.load(prior_tokens + 3 * stride_conv_state_tok, mask=lane_active & mask_w, other=0.0).to(tl.float16)
        if KERNEL_WIDTH >= 6:
            col4 = tl.load(prior_tokens + 4 * stride_conv_state_tok, mask=lane_active & mask_w, other=0.0).to(tl.float16)
        ...
        for idx_token in tl.range(seqlen_run):
            ...
            if KERNEL_WIDTH == 2:
                ...
                col0 = col1; col1 = col2; col2 = matrix_x
            elif KERNEL_WIDTH == 5:
                col0 = col1; col1 = col2; col2 = col3; col3 = matrix_x
            elif KERNEL_WIDTH == 6:
                col0 = col1; col1 = col2; col2 = col3; col3 = col4; col4 = matrix_x
...
def causal_conv1d_update_npu(
    ...
    _causal_conv1d_update_kernel_npu_tiled[grid](
        ...
        num_accepted_tokens, query_start_loc,
        block_idx_last_scheduled_token, initial_state_idx, out,
        batch, dim, seqlen, eff_state_len, num_cache_lines,
