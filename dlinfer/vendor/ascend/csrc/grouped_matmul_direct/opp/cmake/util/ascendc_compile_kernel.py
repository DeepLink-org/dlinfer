...
class CompileKernel:
    ...
    def ascendc_build(self: any):
        ...
        system_site = os.path.join(sys.base_prefix, "lib", python_version, "site-packages")
        compiler_python = "{} -S".format(sys.executable)
        if os.getenv("TILINGKEY_PAR_COMPILE") is None:
            cmd_str = ('export PYTHONPATH=$PYTHONPATH:{0} && export HI_PYTHON="{1}" && '
                       'export ASCEND_CUSTOM_OPP_PATH={{}} && export TILINGKEY_PAR_COMPILE=1'
                       '&& make -f {{}} PY={{}} OUT={{}} CPP={{}}').format(system_site, compiler_python)
        else:
            cmd_str = ('export PYTHONPATH=$PYTHONPATH:{0} && export HI_PYTHON="{1}" && '
                       'export ASCEND_CUSTOM_OPP_PATH={{}} && make -f {{}} PY={{}} OUT={{}} CPP={{}}').format(
                           system_site, compiler_python)
