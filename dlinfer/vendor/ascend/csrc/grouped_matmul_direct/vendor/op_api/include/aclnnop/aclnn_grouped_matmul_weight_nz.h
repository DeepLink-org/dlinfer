...
/**
 * @brief aclnnDlinferGroupedMatmulDirectWeightNz的第一段接口，根据具体的计算流程，计算workspace大小。
 ...
 */
__attribute__((visibility("default"))) aclnnStatus aclnnDlinferGroupedMatmulDirectWeightNzGetWorkspaceSize(
...
/**
 * @brief aclnnDlinferGroupedMatmulDirectWeightNz的第二段接口，用于执行计算。
 * @param [in] workspace: 在npu device侧申请的workspace内存起址。
 * @param [in] workspaceSize: 在npu device侧申请的workspace大小，由第一段接口aclnnDlinferGroupedMatmulDirectWeightNzGetWorkspaceSize获取。
 * @param [in] stream: acl stream流。
 * @param [in] executor: op执行器，包含了算子计算流程。
 * @return aclnnStatus: 返回状态码。
 */
__attribute__((visibility("default"))) aclnnStatus aclnnDlinferGroupedMatmulDirectWeightNz(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                            aclrtStream stream);
