...
typedef aclTensor* (*_aclCreateTensor)(const int64_t* view_dims, uint64_t view_dims_num, aclDataType data_type, const int64_t* stride, int64_t offset,
                                       aclFormat format, const int64_t* storage_dims, uint64_t storage_dims_num, void* tensor_data);
...
inline aclDataType ConvertType(const at::ScalarType scalarType) { return dlinfer::ascend::convert_to_acl_data_type(scalarType); }
...
inline void UnInitCacheThreadLocal() {
    ...
    UnInitPTACacheThreadLocal unInitPTACacheThreadLocalFunc = reinterpret_cast<UnInitPTACacheThreadLocal>(unInitPTACacheThreadLocalAddr);
...
#define EXEC_NPU_CMD(aclnn_api, ...)                                                                                                   \
    do {                                                                                                                               \
        ...
        TORCH_CHECK(workspace_status == 0, "call " #aclnn_api " failed, detail:", aclGetRecentErrMsg(), OPS_ERROR(ErrCode::INTERNAL)); \
        void* workspace_addr = nullptr;                                                                                                \
        if (workspace_size != 0) {                                                                                                     \
            auto workspace_tensor = dlinfer::ascend::unsafe_empty_workspace(workspace_size);                                           \
            workspace_addr = const_cast<void*>(workspace_tensor.storage().data());                                                     \
        ...
        auto acl_call = [converted_params, workspace_addr, workspace_size, acl_stream, executor]() -> int {                            \
            ...
            auto api_ret = opApiFunc(workspace_addr, workspace_size, executor, acl_stream);                                            \
            TORCH_CHECK(api_ret == 0, "call " #aclnn_api " failed, detail:", aclGetRecentErrMsg(), OPS_ERROR(ErrCode::INTERNAL));      \
...
public:
    explicit ConvertedParams(Tuple&& convertedParams, ReleaseHugeMem releaseMemFunc, UnInitHugeMemThreadLocal unInitMemFunc)
        : convertedParams_(std::move(convertedParams)), releaseMemFunc_(releaseMemFunc), unInitMemFunc_(unInitMemFunc){};
    ConvertedParams(ConvertedParams&& other) : convertedParams_(std::move(other.convertedParams_)) { other.validParams_ = false; };
