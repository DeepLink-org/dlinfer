...
torch::Tensor fp8_marlin_gemm(torch::Tensor& a, torch::Tensor& b_q_weight, torch::Tensor& b_scales, torch::Tensor& workspace, int64_t num_bits, int64_t size_m,
...
#define __CALL_IF(NUM_BITS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, NUM_THREADS)                                              \
        ...
        cudaFuncSetAttribute(Marlin<scalar_t, NUM_BITS, NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, pipe_stages, GROUP_BLOCKS>,    \
                             cudaFuncAttributeMaxDynamicSharedMemorySize,                                                                              \
                             max_shared_mem);                                                                                                          \
        Marlin<scalar_t, NUM_BITS, NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, pipe_stages, GROUP_BLOCKS>                          \
            <<<blocks, NUM_THREADS, max_shared_mem, stream>>>(A_ptr, B_ptr, C_ptr, s_ptr, num_groups, prob_m, prob_n, prob_k, locks);                  \
    }
...
int get_scales_cache_size(thread_config_t const& th_config, int prob_m, int prob_n, int prob_k, int num_bits, int group_size) {
...
bool is_valid_config(thread_config_t const& th_config, int max_m_blocks, int prob_m, int prob_n, int prob_k, int num_bits, int group_size, int max_shared_mem) {
    // Sanity
    ...
    int scales_cache_size = get_scales_cache_size(th_config, prob_m, prob_n, prob_k, num_bits, group_size);
...
exec_config_t determine_thread_config(int prob_m, int prob_n, int prob_k, int num_bits, int group_size, int max_shared_mem) {
    int max_m_blocks = 4;
    while (max_m_blocks > 0) {
        if (prob_m <= 16) {
            for (auto th_config : small_batch_thread_configs) {
                if (is_valid_config(th_config, max_m_blocks, prob_m, prob_n, prob_k, num_bits, group_size, max_shared_mem)) {
...
template <typename scalar_t>
void marlin_mm_f16i4(const void* A, const void* B, void* C, void* s, int prob_m, int prob_n, int prob_k, void* workspace, int num_bits, int num_groups,
                     int group_size, int dev, cudaStream_t stream, int thread_k, int thread_n, int sms, int max_par) {
    ...
    if (thread_k != -1 && thread_n != -1) {
        ...
        // Auto config
        exec_cfg = determine_thread_config(prob_m, prob_n, prob_k, num_bits, group_size, max_shared_mem);
    }

    TORCH_CHECK(
        exec_cfg.max_m_blocks > 0 && is_valid_config(exec_cfg.tb_cfg, exec_cfg.max_m_blocks, prob_m, prob_n, prob_k, num_bits, group_size, max_shared_mem),
    ...
    for (int i = 0; i < tot_m_blocks; i += exec_cfg.max_m_blocks) {
        ...
        CALL_IF(8, 4, 8, 128)
            ...
            TORCH_CHECK(false,
                        "Unsupported shapes: MNK = [" + str(prob_m) + ", " + str(prob_n) + ", " + str(prob_k) + "]" + ", num_groups = " + str(num_groups) +
...
torch::Tensor fp8_marlin_gemm(torch::Tensor& a, torch::Tensor& b_q_weight, torch::Tensor& b_scales, torch::Tensor& workspace, int64_t num_bits, int64_t size_m,
    ...
    if (a.scalar_type() == at::ScalarType::Half) {
        ...
        fp8_marlin::marlin_mm_f16i4<nv_bfloat16>(a.data_ptr<at::BFloat16>(),
                                                 b_q_weight.data_ptr(),
