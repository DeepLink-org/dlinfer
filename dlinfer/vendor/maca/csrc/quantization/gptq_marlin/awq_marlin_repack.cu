...
__global__ void awq_marlin_repack_kernel(uint32_t const* __restrict__ b_q_weight_ptr, uint32_t* __restrict__ out_ptr, int size_k, int size_n) {}

}  // namespace marlin

torch::Tensor awq_marlin_repack(torch::Tensor& b_q_weight, torch::Tensor& perm, int64_t size_k, int64_t size_n, int64_t num_bits) {
    ...
    auto fetch_to_shared = [&](int pipe, int k_tile_id, int n_tile_id) {
        ...
        if (threadIdx.x < stage_size) {
            ...
            cp_async4(&sh_ptr[k_id * stage_n_threads + n_id],
                      reinterpret_cast<int4 const*>(&(b_q_weight_ptr[(first_k + k_id) * (size_n / pack_factor) + first_n_packed + (n_id * 4)])));
        }
...
#define CALL_IF(NUM_BITS)                                                                                                                                      \
        ...
        cudaFuncSetAttribute(marlin::awq_marlin_repack_kernel<marlin::repack_threads, NUM_BITS>, cudaFuncAttributeMaxDynamicSharedMemorySize, max_shared_mem); \
...
torch::Tensor awq_marlin_repack(torch::Tensor& b_q_weight, int64_t size_k, int64_t size_n, int64_t num_bits) {
    ...
    TORCH_CHECK(size_k % marlin::tile_k_size == 0, "size_k = ", size_k, " is not divisible by tile_k_size = ", marlin::tile_k_size);
    TORCH_CHECK(size_n % marlin::tile_n_size == 0, "size_n = ", size_n, " is not divisible by tile_n_size = ", marlin::tile_n_size);
