...
template <const int num_bits,          // weight bits
          ...
          >
__global__ void Marlin_24(const int4* __restrict__ A,     // fp16 input matrix of shape mxk
...
torch::Tensor gptq_marlin_24_gemm(torch::Tensor& a, torch::Tensor& b_q_weight, torch::Tensor& b_meta, torch::Tensor& b_scales, torch::Tensor& workspace,
                                  vllm::ScalarTypeTorchPtr const& b_q_type, int64_t size_m, int64_t size_n, int64_t size_k) {
...
__global__ void Marlin_24(const int4* __restrict__ A,     // fp16 input matrix of shape mxk
    ...
    // the middle of group.
    if (group_blocks != -1) iters = (group_blocks / thread_k_blocks) * ceildiv(iters, (group_blocks / thread_k_blocks));
...
#pragma unroll
                            for (int j1 = 0; j1 < 4; j1++) {
                                reinterpret_cast<__half*>(&c)[(j2 * 4 + j1)] =
                                    __float2half(reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 + 4 * ((i % 4) / 2) + i % 2]);
                            }
                        }
    ...
    auto write_result = [&]() {
        ...
        auto write = [&](int idx, float c0, float c1, float c2, float c3, FragS& s0, float c4, float c5, float c6, float c7, FragS& s1) {
            uint2 res[2];
...
#define CALL_IF_2_4(NUM_BITS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS)                                                         \
        ...
        cudaFuncSetAttribute(Marlin_24<NUM_BITS, THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>,                    \
                             cudaFuncAttributeMaxDynamicSharedMemorySize,                                                                              \
                             max_shared_mem);                                                                                                          \
        Marlin_24<NUM_BITS, THREADS, THREAD_N_BLOCKS, THREAD_M_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>                                          \
            <<<blocks, THREADS, max_shared_mem, stream>>>(A_ptr, B_ptr, meta_ptr, C_ptr, s_ptr, prob_n, prob_m, prob_k, locks);                        \
    }

void marlin_cuda_2_4(const void* A, const void* B, const void* meta, void* C, void* s, int prob_m, int prob_n, int prob_k, void* workspace, int num_bits,
                     int groupsize = -1, int dev = 0, cudaStream_t stream = 0, int thread_k = -1, int thread_m = -1, int sms = -1, int max_par = 16) {
    int tot_n = prob_n;
    ...
    if (group_blocks != -1) {
        TORCH_CHECK((prob_k / 2) % group_blocks == 0, "prob_k/2 = ", prob_k / 2, " is not divisible by group_blocks = ", group_blocks);
    }
...
torch::Tensor gptq_marlin_24_gemm(torch::Tensor& a, torch::Tensor& b_q_weight, torch::Tensor& b_meta, torch::Tensor& b_scales, torch::Tensor& workspace,
                                  vllm::ScalarTypeTorchPtr const& b_q_type, int64_t size_m, int64_t size_n, int64_t size_k) {
    // Verify num_bits
    TORCH_CHECK(*b_q_type == vllm::kU4B8 || *b_q_type == vllm::kU8B128, "num_bits must be uint4b8 or uint8b128. Got = ", b_q_type->str());
    ...
    TORCH_CHECK(size_k % marlin_24::tile_size == 0, "size_k = " + str(size_k) + " is not divisible by tile_size = " + str(marlin_24::tile_size));
    TORCH_CHECK((size_k / marlin_24::tile_size / 2) == b_q_weight.size(0),
                "Shape mismatch: b_q_weight.size(0) = " + str(b_q_weight.size(0)) + ", size_k = " + str(size_k) + ", tile_size = " + str(marlin_24::tile_size));

    // Verify N
    TORCH_CHECK(b_scales.size(1) == size_n, "b_scales.size(1) = " + str(b_scales.size(1)) + ", size_n = " + str(size_n));
    TORCH_CHECK(b_q_weight.size(1) % marlin_24::tile_size == 0,
                "b_q_weight.size(1) = " + str(b_q_weight.size(1)) + " is not divisible by tile_size = " + str(marlin_24::tile_size));
    ...
    // Verify meta
    TORCH_CHECK(b_meta.size(0) == size_k / 8 / 2 / 2, "b_meta.size(0) = ", b_meta.size(0), " is not size_k / 8 / 2 / 2 = ", size_k / 8 / 2 / 2);
    ...
    if (b_scales.size(0) > 1) {
        TORCH_CHECK(size_k % b_scales.size(0) == 0, "size_k = " + str(size_k) + ", is not divisible by b_scales.size(0) = " + str(b_scales.size(0)));
    ...
    TORCH_CHECK(size_n % marlin_24::min_thread_n == 0, "size_n = " + str(size_n) + ", is not divisible by min_thread_n = " + str(marlin_24::min_thread_n));
    ...
    marlin_24::marlin_cuda_2_4(a.data_ptr(),
                               b_q_weight.data_ptr(),
                               b_meta.data_ptr(),
                               c.data_ptr(),
                               b_scales.data_ptr(),
