...
__device__ inline FragB dequant_per_group(int q, FragS_GROUP& frag_s, int i) {
    ...
    *reinterpret_cast<half2*>(&t1) = __hfma2(*reinterpret_cast<half2*>(&t1), *reinterpret_cast<const half2*>(&MUL), *reinterpret_cast<const half2*>(&ADD));
    ...
    *reinterpret_cast<half2*>(&t0) = __hfma2(*reinterpret_cast<half2*>(&t0), *reinterpret_cast<half2*>(&double_s), *reinterpret_cast<const half2*>(&MAGIC_NUM));
    *reinterpret_cast<half2*>(&t1) = __hfma2(*reinterpret_cast<half2*>(&t1), *reinterpret_cast<half2*>(&double_s), *reinterpret_cast<const half2*>(&MAGIC_NUM));
...
__global__ void Marlin(const int4* __restrict__ A,        // int8 input matrix of shape mxk
    ...
    // the middle of group.
    if constexpr (group_blocks != -1) iters = (group_blocks / thread_k_blocks) * ceildiv(iters, (group_blocks / thread_k_blocks));
    ...
    if constexpr (group_blocks != -1) {
        s_group_gl_rd = s_group_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) + s_group_sh_stride * slice_col + threadIdx.x;
...
#pragma unroll
    ...
    auto zero_accums = [&]() {
#pragma unroll
        for (int i = 0; i < thread_m_blocks * 4 * 2 * 4; i++) reinterpret_cast<int*>(frag_c)[i] = 0;
    };
...
#pragma unroll
            ...
            if constexpr (group_blocks != -1) {
                if (pipe % (group_blocks / thread_k_blocks) == 0) {
                    ...
                    if (s_group_sh_wr_pred) cp_async4(&sh_s_group_stage[s_group_sh_wr], &s_group[s_group_gl_rd]);
    ...
    auto fetch_to_registers = [&](int k, int pipe) {
        ...
        if constexpr (group_blocks != -1) {
            int4* sh_s_group_stage = sh_s_group + s_group_sh_stage * ((group_blocks / thread_k_blocks) * (pipe / (group_blocks / thread_k_blocks)));
...
#pragma unroll
                        for (int j = 0; j < 4; j++) {
                            reinterpret_cast<int*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)] += reinterpret_cast<int*>(&d_red1)[j];
                        }
#pragma unroll
                        for (int j = 0; j < 4; j++) {
                            reinterpret_cast<int*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * (j + 4) + (i % 4)] += reinterpret_cast<int*>(&d_red2)[j];
                        }
                    }
                    if (!last) {
                        int4 d1, d2;
#pragma unroll
                        for (int j = 0; j < 4; j++) {
                            reinterpret_cast<int*>(&d1)[j] = reinterpret_cast<int*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)];
                        }
#pragma unroll
                        for (int j = 0; j < 4; j++) {
                            reinterpret_cast<int*>(&d2)[j] = reinterpret_cast<int*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * (j + 4) + (i % 4)];
                        }
...
#pragma unroll
                for (int j = 0; j < 4; j++) {
                    int wr = d_sh_wr + 8 * j;
                    write(wr + (4 * d_sh_stride) * 0 + 0, frag_c[i][j][0][0], frag_c[i][j][0][1], frag_s_tok[i][0], frag_s_ch[j / 2][2 * (j % 2) + 0]);
                    write(wr + (4 * d_sh_stride) * 8 + 0, frag_c[i][j][0][2], frag_c[i][j][0][3], frag_s_tok[i][1], frag_s_ch[j / 2][2 * (j % 2) + 0]);
                    write(wr + (4 * d_sh_stride) * 0 + 4, frag_c[i][j][1][0], frag_c[i][j][1][1], frag_s_tok[i][0], frag_s_ch[j / 2][2 * (j % 2) + 1]);
                    write(wr + (4 * d_sh_stride) * 8 + 4, frag_c[i][j][1][2], frag_c[i][j][1][3], frag_s_tok[i][1], frag_s_ch[j / 2][2 * (j % 2) + 1]);
                }
...
#define __CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, NUM_THREADS)                                                                \
    else if (thread_m_blocks == THREAD_M_BLOCKS && thread_n_blocks == THREAD_N_BLOCKS && thread_k_blocks == THREAD_K_BLOCKS && group_blocks == GROUP_BLOCKS && \
             num_threads == NUM_THREADS) {                                                                                                                     \
        cudaFuncSetAttribute(Marlin<NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>,                                     \
                             cudaFuncAttributeMaxDynamicSharedMemorySize,                                                                                      \
                             max_shared_mem);                                                                                                                  \
        Marlin<NUM_THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>                                                           \
            <<<blocks, NUM_THREADS, max_shared_mem, stream>>>(A_ptr, B_ptr, C_ptr, D_ptr, s_tok_ptr, s_ch_ptr, s_group_ptr, prob_m, prob_n, prob_k, locks);    \
    }
...
void marlin_qqq_cuda(const void* A, const void* B, void* C, void* D, void* s_tok, void* s_ch, void* s_group, int prob_m, int prob_n, int prob_k,
                     void* workspace, int groupsize = -1, int dev = 0, cudaStream_t stream = 0, int thread_k = -1, int thread_n = -1, int sms = -1,
                     int max_par = 16) {
    int tot_m = prob_m;
    ...
    if (!is_valid_config(th_config, prob_m, prob_n, prob_k)) {
        throw std::runtime_error("Invalid thread config: thread_k = " + str(th_config.thread_k) + ", thread_n = " + str(th_config.thread_n) +
                                 ", num_threads = " + str(th_config.num_threads) + " for MKN = [" + str(prob_m) + ", " + str(prob_k) + ", " + str(prob_n) +
                                 "]");
    }
    ...
    for (int i = 0; i < tot_m_blocks; i += 4) {
        ...
        CALL_IF(4, 8, 128)
            ...
            throw std::runtime_error("Unsupported shapes: MKN = [" + str(prob_m) + ", " + str(prob_k) + ", " + str(prob_n) + "]" +
                                     ", groupsize = " + str(groupsize) + ", thread_m_blocks = " + str(thread_m_blocks) +
...
torch::Tensor marlin_qqq_gemm(torch::Tensor const& a, torch::Tensor const& b_q_weight, torch::Tensor const& s_tok, torch::Tensor const& s_ch,
                              torch::Tensor const& s_group, torch::Tensor& workspace, int64_t size_m, int64_t size_n, int64_t size_k) {
    // Verify M
    TORCH_CHECK(size_m == a.size(0), "Shape mismatch: a.size(0) = " + str(a.size(0)) + ", size_m = " + str(size_m));
    TORCH_CHECK(size_m == s_tok.numel(), "Shape mismatch: s_tok.numel() = " + str(s_tok.numel()) + ", size_m = " + str(size_m));

    // Verify K
    TORCH_CHECK(size_k == a.size(1), "Shape mismatch: a.size(1) = " + str(a.size(1)) + ", size_k = " + str(size_k));
    TORCH_CHECK(size_k % tile_size == 0, "size_k = " + str(size_k) + " is not divisible by tile_size = " + str(tile_size));
    TORCH_CHECK((size_k / tile_size) == b_q_weight.size(0),
                "Shape mismatch: b_q_weight.size(0) = " + str(b_q_weight.size(0)) + ", size_k = " + str(size_k) + ", tile_size = " + str(tile_size));
    ...
    // Verify N
    TORCH_CHECK(s_ch.numel() == size_n, "Shape mismatch: s_ch.numel() = " + str(s_ch.numel()) + ", size_n = " + str(size_n));
    TORCH_CHECK(b_q_weight.size(1) % tile_size == 0, "b_q_weight.size(1) = " + str(b_q_weight.size(1)) + " is not divisible by tile_size = " + str(tile_size));
    if (groupsize != -1) {
        TORCH_CHECK(s_group.size(1) == size_n, "Shape mismatch: s_group.size(1) = " + str(s_group.size(1)) + ", size_n = " + str(size_n));
        TORCH_CHECK(size_k % s_group.size(0) == 0, "size_k = " + str(size_k) + ", is not divisible by s_group.size(0) = " + str(s_group.size(0)));
    }

    int actual_size_n = (b_q_weight.size(1) / tile_size) * pack_factor_4bit;
    TORCH_CHECK(size_n == actual_size_n, "Shape mismatch: size_n = " + str(size_n) + ", actual_size_n = " + str(actual_size_n));
    ...
    TORCH_CHECK(size_n % min_thread_n == 0, "size_n = " + str(size_n) + ", is not divisible by min_thread_n = " + str(min_thread_n));
