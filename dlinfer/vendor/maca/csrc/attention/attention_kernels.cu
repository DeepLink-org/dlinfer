...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE, int NUM_THREADS, vllm::Fp8KVCacheDataType KV_DTYPE, bool IS_BLOCK_SPARSE,
          int PARTITION_SIZE = 0>                                            // Zero means no partitioning.
__device__ void paged_attention_kernel(float* __restrict__ exp_sums,         // [num_seqs, num_heads, max_num_partitions]
                                       ...
                                       const int q_stride, const int kv_block_stride, const int kv_head_stride, const float k_scale, const float v_scale,
                                       const int tp_rank, const int blocksparse_local_blocks, const int blocksparse_vert_stride,
...
#pragma unroll
    ...
    for (int i = 0; i < NUM_TOKENS_PER_THREAD_GROUP; i++) {
        ...
        const cache_t* k_ptr = k_cache + static_cast<int64_t>(kv_offset0) * kv_block_stride + kv_head_idx * kv_head_stride + physical_block_offset * x;
...
__device__ void paged_attention_kernel_32N(
    ...
    const int max_num_blocks_per_seq,
    const float* __restrict__ alibi_slopes,  // [num_heads]
    const int q_stride, const int kv_block_stride, const int kv_head_stride, const float kv_scale, const int tp_rank, const int blocksparse_local_blocks,
    const int blocksparse_vert_stride, const int blocksparse_block_size, const int blocksparse_head_sliding_step) {
    const int seq_idx = blockIdx.y;
    const int partition_idx = blockIdx.z;
    const int max_num_partitions = gridDim.z;
    constexpr bool USE_PARTITIONING = PARTITION_SIZE > 0;
...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE, int NUM_THREADS, vllm::Fp8KVCacheDataType KV_DTYPE,
          bool IS_BLOCK_SPARSE>
__global__ void paged_attention_v1_kernel(scalar_t* __restrict__ out,           // [num_seqs, num_heads, head_size]
                                          const scalar_t* __restrict__ q,       // [num_seqs, num_heads, head_size]
                                          const cache_t* __restrict__ k_cache,  // [num_blocks, num_kv_heads,
                                                                                // head_size/x, block_size, x]
                                          const cache_t* __restrict__ v_cache,  // [num_blocks, num_kv_heads,
...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE, int NUM_THREADS, vllm::Fp8KVCacheDataType KV_DTYPE,
          bool IS_BLOCK_SPARSE>
__global__ void paged_attention_v1_32N_kernel(scalar_t* __restrict__ out,           // [num_seqs, num_heads, head_size]
                                              const scalar_t* __restrict__ q,       // [num_seqs, num_heads, head_size]
                                              const cache_t* __restrict__ k_cache,  // [num_blocks, num_kv_heads,
                                                                                    // head_size/x, block_size, x]
                                              const cache_t* __restrict__ v_cache,  // [num_blocks, num_kv_heads,
                                              ...
                                              const int max_num_blocks_per_seq,
                                              const float* __restrict__ alibi_slopes,  // [num_heads]
                                              const int q_stride, const int kv_block_stride, const int kv_head_stride, const float kv_scale, const int tp_rank,
                                              const int blocksparse_local_blocks, const int blocksparse_vert_stride, const int blocksparse_block_size,
                                              const int blocksparse_head_sliding_step) {
    paged_attention_kernel_32N<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE>(
        /* exp_sums */ nullptr,
        /* max_logits */ nullptr,
        out,
        q,
...
__global__ void paged_attention_v2_kernel(float* __restrict__ exp_sums,         // [num_seqs, num_heads, max_num_partitions]
                                          ...
                                          const int tp_rank, const int blocksparse_local_blocks, const int blocksparse_vert_stride,
                                          const int blocksparse_block_size, const int blocksparse_head_sliding_step) {
    paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE, PARTITION_SIZE>(exp_sums,
                                                                                                                             max_logits,
                                                                                                                             tmp_out,
                                                                                                                             q,
                                                                                                                             k_cache,
...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE, int NUM_THREADS, vllm::Fp8KVCacheDataType KV_DTYPE, bool IS_BLOCK_SPARSE,
          int PARTITION_SIZE>
__global__ void paged_attention_v2_32N_kernel(float* __restrict__ exp_sums,         // [num_seqs, num_heads, max_num_partitions]
                                              float* __restrict__ max_logits,       // [num_seqs, num_heads,
                                                                                    // max_num_partitions]
                                              scalar_t* __restrict__ tmp_out,       // [num_seqs, num_heads,
                                                                                    // max_num_partitions, head_size]
                                              ...
                                              const int blocksparse_local_blocks, const int blocksparse_vert_stride, const int blocksparse_block_size,
                                              const int blocksparse_head_sliding_step) {
    paged_attention_kernel_32N<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE, PARTITION_SIZE>(exp_sums,
                                                                                                                                 max_logits,
                                                                                                                                 tmp_out,
                                                                                                                                 q,
                                                                                                                                 k_cache,
...
template <typename scalar_t, int HEAD_SIZE, int NUM_THREADS,
          int PARTITION_SIZE>
__global__ void paged_attention_v2_reduce_kernel(scalar_t* __restrict__ out,            // [num_seqs, num_heads, head_size]
...
#define LAUNCH_PAGED_ATTENTION_V1(HEAD_SIZE)                                                                                                   \
    VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                                                                                      \
        ((void*)vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE>), shared_mem_size); \
    vllm::paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE>                                 \
        <<<grid, block, shared_mem_size, stream>>>(out_ptr,                                                                                    \
                                                   query_ptr,                                                                                  \
                                                   key_cache_ptr,                                                                              \
                                                   value_cache_ptr,                                                                            \
                                                   num_kv_heads,                                                                               \
...
#define LAUNCH_PAGED_ATTENTION_V1_32N(HEAD_SIZE)                                                                                                   \
    VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                                                                                          \
        ((void*)vllm::paged_attention_v1_32N_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE>), shared_mem_size); \
    vllm::paged_attention_v1_32N_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE>                                 \
        <<<grid, block, shared_mem_size, stream>>>(out_ptr,                                                                                        \
                                                   query_ptr,                                                                                      \
                                                   key_cache_ptr,                                                                                  \
                                                   value_cache_ptr,                                                                                \
...
template <typename scalar_t>
__global__ void reshape_k_layout_new(scalar_t* __restrict__ k_buffer, scalar_t* k_output, int num_blocks, int num_kv_heads, int head_size, int block_size,
                                     int x, int dst_x) {
...
template <typename CACHE_T, int BLOCK_SIZE>
void reshape_kv_cache(torch::Tensor& key_cache, torch::Tensor& value_cache, torch::Tensor& key_cache_new_layer, torch::Tensor& value_cache_new_layer,
                      int num_seqs, int num_heads, int head_size, int num_kv_heads) {
    ...
    reshape_k_layout_new<CACHE_T><<<dim3(key_cache.size(0) * num_kv_heads, 1, 1), dim3(256, 1, 1), 0, stream>>>(
        key_cache_ptr, key_cache_tmp, key_cache.size(0), num_kv_heads, head_size, BLOCK_SIZE, 8, 16);
    reshape_v_layout<CACHE_T><<<dim3(key_cache.size(0), 1, 1), dim3(256, 1, 1), 0, stream>>>(
        value_cache_ptr, value_cache_tmp, key_cache.size(0), num_kv_heads, head_size, BLOCK_SIZE);
}

#define CALL_RESHAPE_LAUNCHER(CACHE_T, BLOCK_SIZE) \
    reshape_kv_cache<CACHE_T, BLOCK_SIZE>(key_cache, value_cache, key_cache_new_layer, value_cache_new_layer, num_seqs, num_heads, head_size, num_kv_heads);
...
void page_reshape_kv_cache(torch::Tensor& key_cache,              // [num_blocks, num_heads, head_size/x, block_size, x]
                           ...
                           int block_size, const std::string& kv_cache_dtype) {
...
void paged_attention_v1_launcher(torch::Tensor& out, torch::Tensor& query, torch::Tensor& key_cache, torch::Tensor& value_cache, int num_kv_heads, float scale,
                                 torch::Tensor& block_tables, torch::Tensor& seq_lens, int max_seq_len, const c10::optional<torch::Tensor>& alibi_slopes,
                                 float k_scale, float v_scale, const int tp_rank, const int blocksparse_local_blocks, const int blocksparse_vert_stride,
                                 const int blocksparse_block_size, const int blocksparse_head_sliding_step) {
...
#define CALL_V1_LAUNCHER(T, CACHE_T, BLOCK_SIZE, KV_DTYPE, IS_BLOCK_SPARSE)                                  \
    paged_attention_v1_launcher<T, CACHE_T, BLOCK_SIZE, KV_DTYPE, IS_BLOCK_SPARSE>(out,                      \
                                                                                   query,                    \
                                                                                   key_cache,                \
                                                                                   value_cache,              \
                                                                                   num_kv_heads,             \
...
void paged_attention_v1(torch::Tensor& out,          // [num_seqs, num_heads, head_size]
                        ...
                        int64_t block_size, int64_t max_seq_len, const c10::optional<torch::Tensor>& alibi_slopes, const std::string& kv_cache_dtype,
                        double k_scale, double v_scale, const int64_t tp_rank, const int64_t blocksparse_local_blocks, const int64_t blocksparse_vert_stride,
...
#define LAUNCH_PAGED_ATTENTION_V2(HEAD_SIZE)                                                                                   \
    vllm::paged_attention_v2_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE, PARTITION_SIZE> \
        <<<grid, block, shared_mem_size, stream>>>(exp_sums_ptr,                                                               \
                                                   max_logits_ptr,                                                             \
                                                   tmp_out_ptr,                                                                \
                                                   query_ptr,                                                                  \
                                                   key_cache_ptr,                                                              \
...
#define LAUNCH_PAGED_ATTENTION_V2_32N(HEAD_SIZE)                                                                                   \
    vllm::paged_attention_v2_32N_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE, IS_BLOCK_SPARSE, PARTITION_SIZE> \
        <<<grid, block, shared_mem_size, stream>>>(exp_sums_ptr,                                                                   \
                                                   max_logits_ptr,                                                                 \
                                                   tmp_out_ptr,                                                                    \
                                                   query_ptr,                                                                      \
...
void paged_attention_v2_launcher(torch::Tensor& out, torch::Tensor& exp_sums, torch::Tensor& max_logits, torch::Tensor& tmp_out, torch::Tensor& query,
                                 torch::Tensor& key_cache, torch::Tensor& value_cache, int num_kv_heads, float scale, torch::Tensor& block_tables,
                                 torch::Tensor& seq_lens, int max_seq_len, const c10::optional<torch::Tensor>& alibi_slopes, float k_scale, float v_scale,
                                 const int tp_rank, const int blocksparse_local_blocks, const int blocksparse_vert_stride, const int blocksparse_block_size,
                                 const int blocksparse_head_sliding_step) {
    int num_seqs = query.size(0);
    int num_heads = query.size(1);
    int head_size = query.size(2);
...
#define CALL_V2_LAUNCHER(T, CACHE_T, BLOCK_SIZE, KV_DTYPE, IS_BLOCK_SPARSE)                                  \
    paged_attention_v2_launcher<T, CACHE_T, BLOCK_SIZE, KV_DTYPE, IS_BLOCK_SPARSE>(out,                      \
                                                                                   exp_sums,                 \
                                                                                   max_logits,               \
                                                                                   tmp_out,                  \
                                                                                   query,                    \
