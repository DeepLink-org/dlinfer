...
if triton.__version__ >= "2.1.0":
    ...
    def _fwd_kernel(
        ...
        dim_mask = tl.where(
            tl.arange(0, BLOCK_KDMODEL_PADDED) < BLOCK_KDMODEL, 1, 0
        ).to(
            tl.int1
        )  # [D]
        dim_vmask = tl.where(
            tl.arange(0, BLOCK_VDMODEL_PADDED) < BLOCK_VDMODEL, 1, 0
        ).to(
            tl.int1
        )  # [D]
        ...
        off_o = (
            ...
            + cur_head * stride_oh
            + offs_vd[None, :] * stride_od
        )
        out_ptrs = Out + off_o
        tl.store(
            out_ptrs,
            acc,
            mask=dim_vmask[None, :] & (offs_m[:, None] < cur_batch_query_len),
        )
        return
